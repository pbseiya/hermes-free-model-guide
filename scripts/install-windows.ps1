...
if (-not [string]::IsNullOrWhiteSpace($OKMDKey)) {
    ...
    try {
        ...
        $okmdModels = $response.data | Where-Object { $_.id } | Select-Object -ExpandProperty id -First 23
        
        if ($okmdModels.Count -eq 0) {
            throw "No models returned from API"
        }
        
        Write-Ok "Found $($okmdModels.Count) models from OKMD API"
    } catch {
    ...
    $modelsYaml = ""
    $firstModel = ""
    foreach ($modelId in $okmdModels) {
        if (-not $firstModel) {
            $firstModel = $modelId
        }
        $modelsYaml += "      ${modelId}:`n        context_length: 1000000`n"
    }
...
"@
    ...
    Write-Ok "Default model: $firstModel"
...
"@
    ...
    Write-Info "Default model: $firstModel"
...
"@
    ...
    # Update config.yaml to use LiteLLM with network binding and logging
    $configContent = @"
# Hermes Agent Configuration
